      ...
      <div className="flex flex-col gap-4 border-b border-cream-300 pb-6">
        <div>
          ...
          <p className="text-sm text-charcoal-500">
            Real interview experiences from seniors, and a preparation
            leaderboard to keep you consistent.
          </p>
        </div>
  ...
  const fetchExperiences = async () => {
    ...
    try {
      const url = companyFilter
        ? `/api/experiences?company=${companyFilter}`
        : "/api/experiences";
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p className="text-xs text-charcoal-500">
          Read hyper-local, real questions asked at your college, shared by
          seniors.
        </p>
        {token && (
          <button
        ...
        <div className="bg-cream-200 border border-cream-300 rounded-xl p-6 shadow-paper flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-4 pl-1">
              ...
              <label
                htmlFor="anon"
                className="text-xs font-semibold text-charcoal-900 cursor-pointer"
              >
      ...
      {loading ? (
        <div className="text-center py-16 text-xs text-charcoal-100 italic">
          ...
          {experiences.map((exp) => (
            ...
            >
              ...
              <div className="flex flex-col gap-2 bg-white p-4 rounded-lg border border-cream-300">
                <span className="text-[9px] font-bold text-sage-500 uppercase tracking-wider">
              ...
              {exp.tips && (
                <div className="text-xs text-charcoal-500 bg-cream-50 border border-cream-300/40 p-3 rounded-lg leading-relaxed">
                  <strong className="text-charcoal-900">Advice:</strong>{" "}
                  {exp.tips}
                </div>
              )}
            </div>
        ...
        <div className="bg-cream-200 border border-cream-300 rounded-xl p-16 shadow-paper text-center flex flex-col items-center justify-center gap-2">
          ...
          <h3 className="font-serif font-bold text-charcoal-900">
            No logs found
          </h3>
          <p className="text-xs text-charcoal-500 max-w-xs mx-auto">
            No peer experiences posted for this company yet. Click "Share Your
            Experience" to be the first!
          </p>
        </div>
      )}
    </div>
...
function PeerChallengeTab({ user, token }) {
  ...
  const handleSaveSettings = async (optInValue) => {
    ...
    if (optInValue && handleInput.trim().length < 3) {
      setSettingsError(
        "Please choose a display handle (at least 3 characters) first.",
      );
      return;
    }
    ...
    try {
      const res = await fetch("/api/auth/leaderboard-settings", {
        ...
        body: JSON.stringify({
          displayHandle: handleInput.trim(),
  ...
  if (!token) {
    return (
      <div className="bg-cream-200 border border-cream-300 rounded-xl p-16 shadow-paper text-center flex flex-col items-center justify-center gap-2">
        ...
        <p className="text-xs text-charcoal-500">
          Log in to see your preparation stats and join the leaderboard.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatChip
          ...
          value={`${myStats?.currentStreak || 0} days`}
        />
        <StatChip
          icon={<TrendingUp size={14} className="text-sage-500" />}
          label="Weekly Score"
          value={myStats?.weeklyScore || 0}
        />
        <StatChip
          icon={<Target size={14} className="text-terracotta-500" />}
          label="Roadmap Progress"
          value={`${myStats?.roadmapCompletionPct || 0}%`}
        />
        <StatChip
          icon={<Award size={14} className="text-sage-500" />}
          label="Questions Practiced"
          value={myStats?.totalQuestionsAttempted || 0}
        />
      </div>
      ...
      {!myStats?.leaderboardOptIn ? (
        <div className="bg-cream-200 border border-cream-300 rounded-xl p-8 shadow-paper flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h3 className="font-serif font-bold text-charcoal-900 text-lg">
              Join the Peer Challenge
            </h3>
            <p className="text-xs text-charcoal-500 max-w-md mx-auto mt-1">
              Compete with students preparing for the same target company. If
              you join, other students will only ever see: your chosen{" "}
              <strong>display handle</strong>, your preparation score, streak,
              and target company — never your real name, email, or college.
            </p>
          </div>
          ...
          {settingsError && (
            <p className="text-[10px] text-red-600">{settingsError}</p>
          )}
          <div className="flex gap-2">
            <button
      ...
      {showSettings && myStats?.leaderboardOptIn && (
        <div className="bg-cream-200 border border-cream-300 rounded-xl p-6 shadow-paper flex flex-col gap-3">
          ...
          <p className="text-[10px] text-charcoal-100">
            Leaving keeps all your personal stats tracked privately — you just
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex bg-white border border-cream-300 rounded-lg overflow-hidden">
          ...
          <button
            ...
            title={
              !myStats?.targetCompany ? "Select a target company first" : ""
            }
          ...
          >
            <Target size={12} /> Same Target{" "}
            {myStats?.targetCompany ? `(${myStats.targetCompany})` : ""}
          </button>
        </div>
        <div className="flex bg-white border border-cream-300 rounded-lg overflow-hidden">
          <button
          ...
          <table className="w-full text-xs min-w-[480px]">
            ...
            <tbody>
              {leaderboard.map((entry) => {
                const isMe =
                  myStats?.displayHandle &&
                  entry.displayHandle === myStats.displayHandle;
                return (
                  <tr
                    key={entry.rank}
                    className={`border-b border-cream-300/50 ${isMe ? "bg-sage-100/60 font-semibold" : ""}`}
                  >
                    <td className="px-4 py-3 font-serif font-bold text-sage-600">
                      #{entry.rank}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="inline-flex items-center gap-1 text-terracotta-600">
                        <Flame size={11} className="fill-terracotta-500" />
                        {entry.streak}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-charcoal-500">
                      {entry.roadmapCompletionPct}%
                    </td>
  ...
  return (
    <div className="bg-cream-200 border border-cream-300 rounded-xl p-3 shadow-paper flex flex-col gap-1">
      <div className="flex items-center gap-1.5">
        {icon}
        <span className="text-[9px] text-charcoal-100 font-semibold uppercase tracking-wider">
          {label}
        </span>
      </div>
