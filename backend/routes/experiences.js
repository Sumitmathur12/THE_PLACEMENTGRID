...
router.post("/add", requireAuth, async (req, res) => {
  const { role, company, questionsAsked, difficulty, tips, anonymous } =
    req.body;
  ...
  if (!role || !company || !questionsAsked || !Array.isArray(questionsAsked)) {
    return res
      .status(400)
      .json({
        error: "Missing required fields: role, company, or questionsAsked",
      });
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Failed to log experience-post activity:",
        activityErr.message,
      );
    }
  ...
  try {
    ...
    if (scope === "target") {
      ...
      if (!targetCompanyName) {
        return res
          .status(400)
          .json({ error: "scope=target requires a ?company= parameter." });
      }
    ...
    if (leaderboard.length > 0) {
      if (page === 1) {
        badges.weeklyChampion =
          period === "weekly" ? leaderboard[0].displayHandle : null;
        badges.allTimeChampion =
          period === "alltime" ? leaderboard[0].displayHandle : null;
      }
      const mostConsistent = [...leaderboard].sort(
        (a, b) => b.streak - a.streak,
      )[0];
      if (mostConsistent && mostConsistent.streak > 0)
        badges.mostConsistent = mostConsistent.displayHandle;

      const questionCrusher = [...leaderboard].sort(
        (a, b) => b.weeklyQuestionsAttempted - a.weeklyQuestionsAttempted,
      )[0];
      if (questionCrusher && questionCrusher.weeklyQuestionsAttempted > 0)
        badges.questionCrusher = questionCrusher.displayHandle;

      const roadmapFinishers = leaderboard
        .filter((e) => e.roadmapCompletionPct >= 100)
        .map((e) => e.displayHandle);
      if (roadmapFinishers.length > 0)
        badges.roadmapFinishers = roadmapFinishers;
