...
const signToken = (userId) => {
  return Buffer.from(
    JSON.stringify({ id: userId, exp: Date.now() + 24 * 60 * 60 * 1000 }),
  ).toString("base64");
};
  ...
  try {
    ...
    if (token.includes(".")) {
      ...
      if (process.env.NODE_ENV !== "production" && jwtSecret) {
        try {
          ...
          console.log(
            "Local JWT verification failed or bypassed, falling back to Supabase remote API...",
          );
        }
      }
      ...
      try {
        ...
        email =
          decoded.email ||
          (decoded.id === "guest-user-id" ? "guest@college.edu" : decoded.id);
        name =
          decoded.user_metadata?.full_name ||
          decoded.name ||
          "Scholar Candidate";
    ...
    if (!email) {
      return res
        .status(401)
        .json({
          error: "Unauthorized: Token did not contain a valid identifier",
        });
    }
    ...
    return res
      .status(401)
      .json({ error: `Unauthorized: Invalid session. ${error.message}` });
  }
};
...
router.post("/register-sync", async (req, res) => {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    try {
      if (token.includes(".")) {
        ...
        if (process.env.NODE_ENV !== "production" && jwtSecret) {
          try {
            ...
            console.log(
              "register-sync: Local JWT verification failed or bypassed, falling back to Supabase remote API...",
            );
          }
        }
      ...
      console.warn(
        "register-sync: Ignored token decoding failure:",
        err.message,
      );
    }
  }
  ...
  if (!email) {
    return res
      .status(400)
      .json({
        error: "Unauthorized: Email is missing from token payload and body.",
      });
  }
...
router.post("/complete-profile", requireAuth, async (req, res) => {
  ...
  if (!collegeName || !branch || !rollNumber) {
    return res
      .status(400)
      .json({ error: "College Name, Branch, and Roll Number are required." });
  }
...
router.post("/heartbeat", requireAuth, async (req, res) => {
  try {
    ...
    const remainingCap = Math.max(
      0,
      CONFIG.dailyActiveMinutesCap - (activity.genuineActiveMinutes || 0),
    );
    ...
    return res.json({
      ...
      dailyTotal: activity.genuineActiveMinutes,
    });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
  ...
  try {
    ...
    if (displayHandle !== undefined) {
      ...
      if (trimmed.length > 0 && trimmed.length < 3) {
        return res
          .status(400)
          .json({ error: "Display handle must be at least 3 characters." });
      }
    ...
    if (leaderboardOptIn !== undefined) {
      if (leaderboardOptIn === true && !user.displayHandle) {
        return res
          ...
          .json({
            error:
              "Please choose a display handle before joining the leaderboard.",
          });
      }
