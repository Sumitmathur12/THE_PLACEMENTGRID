...
router.post("/start", requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `MockInterview: Starting session. Querying live web search and local RAG for ${cleanCompany} questions...`,
    );

    // 1. Retrieve local company context
    const localContext = await ragRetrieve(
      `${cleanCompany} technical HR interview questions patterns`,
      3,
    );
    ...
    let webContext = "";
    try {
      webContext = await runWebSearch(
        `${cleanCompany} real interview questions asked Glassdoor LeetCode discuss`,
      );
    } catch (searchErr) {
      console.warn(
        "MockInterview: Live Custom Search query bypassed:",
        searchErr.message,
      );
    }
...
Generate the questions.`;
    ...
    const questionsText = await generateLLMResponse(
      systemPrompt,
      contextPrompt,
    );
...
router.post("/submit-answer", requireAuth, async (req, res) => {
  ...
  if (!sessionId || !question || answer === undefined) {
    return res
      .status(400)
      .json({ error: "SessionId, question, and answer are required." });
  }
  ...
  try {
    ...
    if (proctorLogs && Array.isArray(proctorLogs)) {
      proctorLogs.forEach((log) => {
        ...
        session.proctoringIntegrityScore = Math.max(
          0,
          session.proctoringIntegrityScore - deduction,
        );
      });
    }
