...
const makeTestUser = async (label) => {
  ...
  const token = jwtSecret
    ...
    : Buffer.from(JSON.stringify({ ...payload, id: `test-${label}` })).toString('base64');
...
const run = async () => {
  ...
  const statsA0 = await authedGet(`${BASE_URL}/experiences/my-stats`, userA.token);
  ...
  check('leaderboardOptIn defaults to false', statsA0.leaderboardOptIn === false);
  ...
  check('Streak remains 0 after signup with no other action', statsA0.currentStreak === 0);

  // ------------------------------------------------------------------
  // Test 3: Cannot opt into leaderboard without a display handle
  // ------------------------------------------------------------------
  console.log('\n3. Opt-in requires a display handle first...');
  const badOptIn = await authedPost(`${BASE_URL}/auth/leaderboard-settings`, userA.token, {
  ...
  const optInRes = await authedPost(`${BASE_URL}/auth/leaderboard-settings`, userA.token, {
    displayHandle: handleA,
    leaderboardOptIn: true
  });
  check('Opt-in succeeds once a handle is provided', optInRes.ok === true);
  check('Returned displayHandle matches what was set', optInRes.data.displayHandle === handleA);
  ...
  const lbRaw = await fetch(`${BASE_URL}/experiences/leaderboard?scope=global&period=weekly&limit=50`).then(r => r.text());
  check('Response does not contain the test user\'s real name', !lbRaw.includes(userA.name));
  check('Response does not contain the test user\'s email', !lbRaw.includes(userA.email));
  ...
  await authedPost(`${BASE_URL}/auth/leaderboard-settings`, userB.token, { displayHandle: handleB });
  const lbAfterB = await fetch(`${BASE_URL}/experiences/leaderboard?scope=global&period=weekly&limit=50`).then(r => r.json());
  ...
  check('Immediate second heartbeat call is credited 0 (rate-limited)', hb2.data.credited === 0);

  // ------------------------------------------------------------------
  // Test 8: My-stats works and reflects opted-in state
  // ------------------------------------------------------------------
  console.log('\n8. My-stats reflects current opt-in/handle state...');
  const statsA1 = await authedGet(`${BASE_URL}/experiences/my-stats`, userA.token);
  check('my-stats shows leaderboardOptIn true after opting in', statsA1.leaderboardOptIn === true);
  check('my-stats shows the chosen displayHandle', statsA1.displayHandle === handleA);
  ...
  await authedPost(`${BASE_URL}/auth/leaderboard-settings`, userA.token, { leaderboardOptIn: false });
  const lbAfterLeave = await fetch(`${BASE_URL}/experiences/leaderboard?scope=global&period=weekly&limit=50`).then(r => r.json());
  const stillFoundA = lbAfterLeave.leaderboard.find(e => e.displayHandle === handleA);
  check('User no longer appears in public leaderboard after opting out', !stillFoundA);
  const statsA2 = await authedGet(`${BASE_URL}/experiences/my-stats`, userA.token);
  check('Private stats (streak/score) are still tracked after opting out', typeof statsA2.currentStreak === 'number');

  // ------------------------------------------------------------------
  // Test 10: scope=target requires a company param
  // ------------------------------------------------------------------
  console.log('\n10. scope=target requires an explicit company filter...');
  const targetNoCompany = await fetch(`${BASE_URL}/experiences/leaderboard?scope=target`);
  check('Rejects scope=target without a company param', targetNoCompany.status === 400);

  // ------------------------------------------------------------------
  // Test 11: Pagination works
  // ------------------------------------------------------------------
  console.log('\n11. Pagination returns consistent metadata...');
  const page1 = await fetch(`${BASE_URL}/experiences/leaderboard?scope=global&period=weekly&page=1&limit=1`).then(r => r.json());
  check('limit=1 returns at most 1 entry', page1.leaderboard.length <= 1);
  check('totalPages is present and numeric', typeof page1.totalPages === 'number');
  ...
  try {
    ...
    if (mongoUri) {
      ...
      const delResult = await User.deleteMany({ email: { $regex: '^verifylb_' } });
