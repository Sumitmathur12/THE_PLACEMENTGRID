...
export const getISTDateOffsetString = (offsetDays) => {
  ...
  const shifted = new Date(
    utc + 3600000 * 5.5 + offsetDays * 24 * 60 * 60 * 1000,
  );
 ...
 */
export const logGenuineActivity = async (
  userId,
  actionType,
  count = 1,
  metadata = {},
...
export const recalculateStreakAndScore = async (userId) => {
  ...
  let targetRoadmap = await Roadmap.findOne({ userId: user._id }).sort({
    updatedAt: -1,
  });
      ...
      if (w.topics) {
        w.topics.forEach((t) => {
          ...
          if (
            ...
            user.completedRoadmapTopics.includes(t.topicId)
          ) {
            completedTasks++;
          }
        });
  ...
  weeklyActivities.forEach((a) => {
    ...
    const isDayActive =
      ...
      a.roadmapTasksCompleted > 0 ||
      a.mockInterviewsCompleted > 0 ||
      a.resumeActivities > 0 ||
      a.genuineActiveMinutes > 0;
    if (isDayActive) {
      weeklyActiveDays++;
    }
  });
  ...
  const questionScore =
    Math.min(weeklyUniqueQuestionsAttempted / CONFIG.weeklyQuestionTarget, 1) *
    100;
  const roadmapScore =
    Math.min(weeklyRoadmapTasksCompleted / CONFIG.weeklyRoadmapTarget, 1) * 100;
  const activeTimeScore =
    Math.min(weeklyActiveMinutes / CONFIG.weeklyActiveTimeTarget, 1) * 100;
  ...
  if (weeklyQuestionsAttempted > 0) {
    accuracyScore =
      ((weeklyCorrectAnswers + 1) / (weeklyQuestionsAttempted + 2)) * 100;
  ...
  const allTimeStreakScore =
    Math.min((user.longestStreak || 0) / CONFIG.longestStreakTarget, 1) * 100;
  const allTimeQuestionScore =
    Math.min(
      (user.totalQuestionsAttempted || 0) / CONFIG.totalQuestionsTarget,
      1,
    ) * 100;
  ...
  const lifetimeActiveMinutes =
    lifetimeTimeAgg.length > 0 ? lifetimeTimeAgg[0].totalMinutes : 0;
  const allTimeActiveTimeScore =
    Math.min(lifetimeActiveMinutes / CONFIG.totalActiveTimeTarget, 1) * 100;

  user.allTimeScore = Math.round(
    allTimeStreakScore * 0.25 +
      allTimeQuestionScore * 0.3 +
      roadmapCompletion * 0.3 +
